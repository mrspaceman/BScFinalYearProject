...
Procedure Simulate is
    ...
    nodeMuber, nodePort, cycleNumber, x, y         : integer :=1;
    cellReady, nextFreeTransit, TransitCellToSwitch: integer :=1
------------------------------------------------------------------------------

Procedure swtup_screen is
Begin
  my_io.display.cls;
...
Begin
  ...
  for nodeNumber in 1..MaxNumberOfNodes loop
    for y in 1..NoOfPorts loop
      links(nodeMuber,y).Port  := random(NoOfPorts)+1;
      links(nodeMuber,y).Switch:= random(MaxNumberOfNodes)+1;
    end loop;
  end loop;
  ...
  loop   -- One loop is a single time cycle
    ...
    for nodeNumber in 1..MaxNumberOfNodes loop
        ...
        for nodePort in 1..NoOfPorts loop
            ...
            if transitCellToSwtch >0 then
              NetworkNodes(nodeNumber).CellArrival(NewCell);
              for x in transitCellToSwtch..nextFreeTransit loop
                CellsInTransit(x):=CellsInTransit(x+1);
              end loop;
            else
              NewCell := GeneratedCell(nodePort);
              NetworkNodes(nodeNumber).CellArrival(NewCell);
            end if;
      end loop;
      my_io.fileInOut.put_line(fd);
